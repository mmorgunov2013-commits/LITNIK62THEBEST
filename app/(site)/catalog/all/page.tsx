...
export const metadata: Metadata = {
  title: "Каталог — полная витрина",
  description:
    "Вся номенклатура: поиск по названию и марке, фильтр по категории, сортировка.",
};
...
export default async function CatalogAllPage({ searchParams }: Props) {
  ...
  const where: Prisma.ProductWhereInput = {
    ...
    ...(q
      ? {
          OR: [{ title: { contains: q } }, { alloyMark: { contains: q } }],
        }
      : {}),
  ...
  const products = await prisma.product.findMany({
    ...
    orderBy:
      sort === "price" ? { pricePerKg: order } : { title: order },
  });
  ...
  const buildHref = (patch: Record<string, string | undefined>) => {
    ...
    const s = p.toString();
    return s ? `/catalog/all?${s}` : "/catalog/all";
  };
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <p className="text-sm text-[var(--muted)]">
        ...
        <span className="text-[var(--text)]">Вся номенклатура</span>
      </p>
